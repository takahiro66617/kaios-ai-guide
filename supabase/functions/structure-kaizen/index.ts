...
重要：related_departments は直接的・間接的に効果を及ぼす可能性がある全ての部署を推定して含めてください。`,
    ...
    // 人が入力した金額を絶対基準として、合計スコアを band の範囲に強制的にクランプ
    if (band && typeof structured.impact_score === "number") {
      const before = structured.impact_score;
      const clamped = Math.max(band.min, Math.min(band.max, before));
      if (clamped !== before) {
        structured.impact_score = clamped;
        // per_axis_scores も合計が clamped に一致するよう一様スケーリング
        if (Array.isArray(structured.per_axis_scores) && structured.per_axis_scores.length > 0) {
          const targetSum = (clamped / 100) * totalWeight;
          const currentSum = structured.per_axis_scores.reduce(
            (s: number, x: any) => s + (Number(x.score) || 0), 0
          );
          if (currentSum > 0) {
            const ratio = targetSum / currentSum;
            structured.per_axis_scores = structured.per_axis_scores.map((x: any) => ({
              key: x.key,
              score: Math.round((Number(x.score) || 0) * ratio),
            }));
          }
        }
        structured.score_reason = `${structured.score_reason ?? ""}（入力金額帯 ${band.min}〜${band.max}点にクランプ：${band.label}）`;
      }
    }
