...
【採点方針 — 厳格運用（必ず守ること）】
- 合計スコアは「平均45点・大半が25〜65点に収まる」分布を目標とする。
- 軸ごとの目安（0〜ウェイト点）：
  - 0〜30%: 入力が薄い／根拠が乏しい／一般論のみ
  - 30〜55%: 標準的な改善（ほとんどの提案はここ）
  - 55〜75%: 具体性・定量根拠・再現性が十分に揃っている
  - 75〜90%: 全社/事業へ波及する戦略級。社内ベストプラクティス候補
  - 90〜100%: 経営インパクトが極めて大きい突出案件（年に数件レベル）
- 「課題・原因・解決策・効果」のいずれかが曖昧・抽象・未記入なら、その軸は満点の50%以下に抑える。
- 数値根拠（金額・時間・件数・%）がない場合、収益寄与性は満点の40%以下に抑える。
- 安易な高得点（80点超）を避け、突出した提案にのみ与えること。スコアインフレを起こさないこと。
